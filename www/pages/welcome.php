...
*/
  ...
  /**
   * Speichert die Men&uuml;konfiguration des aktuellen Benutzers.
   *
   * @return void
   */
  protected function HandleMenuConfiguratorSave()
  {
      if (!$this->app->Container->has('MenuConfiguratorService')) {
          return;
      }
      ...
      $menu = $this->app->erp->Navigation();
      $allowedKeys = $menuConfigurator->collectKeys($menu);
      ...
      if ($payload !== null && $payload !== '') {
          ...
          if (!is_array($decoded)) {
              $this->app->Tpl->Set(
                  'MESSAGE',
                  $this->app->erp->Fehlermeldung('Men&uuml;konfiguration konnte nicht gespeichert werden. Eingaben waren nicht g&uuml;ltig.')
              );
              return;
          }
      ...
      try {
          $menuConfigurator->saveConfig((int)$this->app->User->GetID(), $config);
          $this->app->Tpl->Set('MESSAGE', $this->app->erp->Meldung('Men&uuml;konfiguration gespeichert.'));
      } catch (\Exception $e) {
          $this->app->Tpl->Set(
              'MESSAGE',
              $this->app->erp->Fehlermeldung('Men&uuml;konfiguration konnte nicht gespeichert werden.')
          );
      }
  }

  /**
   * Rendert den Men&uuml;konfigurator-Block in den Welcome-Settings.
   *
   * @return void
   */
  ...
  {
      if (!$this->app->Container->has('MenuConfiguratorService')) {
          $this->app->Tpl->Set('MENU_CONFIGURATOR', '');
      ...
      $menu = $this->app->erp->Navigation();
      $config = $menuConfigurator->getConfig((int)$this->app->User->GetID());
      ...
      foreach ($menu as $entry) {
          ...
          $firstKey = $menuConfigurator->getKey($firstTitle);
          $firstConfig = isset($configItems[$firstKey]) && is_array($configItems[$firstKey]) ? $configItems[$firstKey] : [];
          $rows[] = $this->buildMenuConfiguratorRow($firstKey, $firstTitle, $firstConfig, 'Hauptmen&uuml;');
          if (isset($entry['sec']) && is_array($entry['sec'])) {
              foreach ($entry['sec'] as $sec) {
                  $secKey = $menuConfigurator->getKey($firstTitle, $sec);
